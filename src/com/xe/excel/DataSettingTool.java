...
import com.xe.tool.impl.ClassTool;
import com.xe.tool.impl.GenericsUtils;
import com.xe.tool.mark.Type;
import com.xe.tool.mark.XmlLabel;
import com.xe.xml.ClassManager;

/**
 * 
 * @Create time : 2014-5-26
 * @author : zhaowei
 * @Description : TODO 配置解析(包含地图配置)
 */
public class DataSettingTool {
	...
	public static <T> List<T> parseByInputStream(InputStream inputStream) throws Exception {
		...
		List<T> settingDatas = new ArrayList<>();
		try {
			// 创建SAXBuilder对象
	...
	private static Object getOne(String context, int arrNum, Class<?> typeClass) {
		...
		} catch (NoSuchMethodException | SecurityException | ArrayIndexOutOfBoundsException | IllegalArgumentException
				| IllegalAccessException | InvocationTargetException | NegativeArraySizeException
